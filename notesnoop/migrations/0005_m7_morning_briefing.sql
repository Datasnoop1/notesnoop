...
CREATE OR REPLACE FUNCTION disable_morning_briefing(
...
CREATE OR REPLACE FUNCTION disable_morning_briefing_by_email(
  target_email TEXT
) RETURNS INTEGER
LANGUAGE plpgsql
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION disable_morning_briefing(UUID, TEXT) TO notesnoop_app, notesnoop_worker;
GRANT EXECUTE ON FUNCTION disable_morning_briefing_by_email(TEXT) TO notesnoop_app, notesnoop_worker;

COMMENT ON FUNCTION disable_morning_briefing(UUID, TEXT) IS
  'Unauthenticated one-click unsubscribe helper. Narrow SECURITY DEFINER path only flips the Morning briefing opt-in flag.';
COMMENT ON FUNCTION disable_morning_briefing_by_email(TEXT) IS
