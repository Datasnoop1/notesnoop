...
- `pytest backend/tests/test_person_v1.py backend/tests/test_ownership_id.py backend/tests/test_ownership_graph_sql.py backend/tests/test_nbb_governance_durability.py -q`: 25 passed.
...
- Final hardening recheck at branch tip `156bf2d`: focused regression suite
  passed with 25 tests; py_compile and UTF-8/BOM/mojibake scan remained clean.
...
Final branch-tip re-smoke after retry hardening:
...
indexes: governance_load_log_pkey,idx_governance_load_retry
smoke_row: ('ok', 1, True)
retry_failed_governance dry-run: retry candidates: 0
```

## Prod gate

Gate Y tail executed 2026-05-02 after the PR comment posted the exact command.
...
backend: healthy
nbb-backload-worker: healthy
managed cron: retry_failed_governance.py installed at 07:45 UTC
governance_load_log_exists: True
indexes: governance_load_log_pkey,idx_governance_load_retry
smoke_row: ('ok', 1, True)
retry_failed_governance dry-run: retry candidates: 0
...
The prod smoke wrote a synthetic retry-log failure row, recorded success for
the same key, verified the final `status='ok'` row, and deleted it.
