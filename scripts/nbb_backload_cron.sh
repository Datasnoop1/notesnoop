...
MAX_CALLS="${MAX_CALLS:-5000}"
START_YEAR="${START_YEAR:-2024}"
END_YEAR="${END_YEAR:-2022}"
PER_YEAR_CAP="${PER_YEAR_CAP:-3000}"
...
log "starting nbb backload (max_calls=$MAX_CALLS start_year=$START_YEAR end_year=$END_YEAR per_year_cap=$PER_YEAR_CAP)"
docker exec -e PYTHONPATH="$PYTHONPATH_IN_CONTAINER" "$BACKEND_CONTAINER" \
    python /app/scripts/nbb_nightly_backload.py \
    --max-calls "$MAX_CALLS" \
    --start-year "$START_YEAR" \
    --end-year "$END_YEAR" \
    --per-year-cap "$PER_YEAR_CAP" >> "$LOG_FILE" 2>&1
exit_code=$?
