...
filtered="$(printf '%s\n' "$current" | grep -v '# notesnoop-postgres-' || true)"
{
  ...
  printf '42 2 */2 * * NOTESNOOP_ENV_FILE=%s bash /opt/leadpeek/scripts/notesnoop_take_base_backup.sh >> %s/notesnoop_base_backup.log 2>&1 # notesnoop-postgres-base-backup\n' "$ENV_FILE" "$LOG_DIR"
