...
refresh_snapshot() {
  ...
  # restored data. Build it now CONCURRENTLY so the staging environment matches
  # prod's vector-search query plan — without this step, find-similar latency
  # tests on staging are not representative of production.
