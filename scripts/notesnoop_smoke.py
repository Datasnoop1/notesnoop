...
def run(base_url: str, basic_auth: str | None) -> None:
    ...
    reminder_rows = data(owner.get(f"/api/workspaces/{workspace_id}/reminders"))
    reminder = next((item for item in reminder_rows if item["task_id"] == task["id"]), None)
    assert_true(reminder is not None, "task due date creates reminder memory")
    owner.patch(f"/api/task-reminders/{reminder['id']}", {"state": "snoozed", "snoozed_until": "2026-05-16T09:00:00Z"})
    snoozed_rows = data(owner.get(f"/api/workspaces/{workspace_id}/reminders"))
    assert_true(any(item["id"] == reminder["id"] and item["state"] == "snoozed" for item in snoozed_rows), "task reminder can be snoozed")
    owner.patch(f"/api/task-reminders/{reminder['id']}", {"state": "dismissed"})
    dismissed_rows = data(owner.get(f"/api/workspaces/{workspace_id}/reminders"))
    ...
    task_brief = data(owner.get(f"/api/briefs/task/{task['id']}?variant=full"))["markdown"]
    meeting_brief = data(owner.get(f"/api/briefs/meeting/{meeting['id']}?variant=quick"))["markdown"]
    report_brief = data(owner.get(f"/api/briefs/report/{report['id']}?variant=quick"))["markdown"]
    ...
    assert_true(
        "Projects:" in task_brief and "People:" in meeting_brief and "Linked tasks:" in report_brief,
        "structured memory copy briefs synthesize graph links",
    )
